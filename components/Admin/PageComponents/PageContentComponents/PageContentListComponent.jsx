...
const PageContentListComponent = ({ token }) => {
  //helper-variables
  ...
  const deletePage = (id) => {
    const apiUrl = BASE_URL + "api/v1/page-contents/delete";
    const apiData = {
      id,
    };
            ...
            <tbody>
              {pageList?.map((pageContent, index) => (
                <tr key={index}>
                  ...
                  <td>{pageContent.page?.name}</td>

                  <td>
                    <Link href={`/admin/pages/page-contents/${pageContent.id}`} className="anchor">
...
export default connect(mapStateToProps)(PageContentListComponent);
