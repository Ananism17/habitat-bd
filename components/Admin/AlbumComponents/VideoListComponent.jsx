...
const VideoListComponent = ({ token }) => {
  //helper-variables
  ...
  //album-list
  useEffect(() => {
    const apiUrl = BASE_URL + "api/v1/albums?type=video&page=" + page;
    axios
      .get(apiUrl, {
            ...
            <tbody>
              {albumList?.map((album, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <Link
                      href={`/admin/albums/video-albums/${album.id}`}
                      className="anchor"
                    >
...
export default connect(mapStateToProps)(VideoListComponent);
